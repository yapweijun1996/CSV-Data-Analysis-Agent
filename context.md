...
- **Follow-up**: Deterministic cleanup now reports both context-row removal and summary-row removal into the workflow timeline via `completeWorkflowStep`, keeping Raw Data Explorer actions auditable. `remove_leading_rows` no longer hardcodes keywords—tokens are derived from metadata header rows + agent-provided `keywords` args, so multi-language titles/headers can be removed without embedding constant strings, and the Title → Header → Data stage log scaffolding handles the deterministic pass before any AI JS executes.
- **Open Questions / TODO**:
