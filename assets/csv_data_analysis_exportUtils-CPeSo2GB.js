...
const appendExportMetadata = (root, metadata) => {
...
const withExportMetadataColumns = (data, metadata) => {
...
const exportToPng = async (element, title, options) => {
  ...
  try {
    ...
    restoreMetadata == null ? void 0 : restoreMetadata();
...
const exportToCsv = (data, title, metadata) => {
  try {
    const csv = papaparse_minExports.unparse(withExportMetadataColumns(data, metadata));
...
const exportDatasetToCsv = (data, fileName) => {
  try {
...
const exportToHtml = async (element, title, data, summary, metadata) => {
  try {
